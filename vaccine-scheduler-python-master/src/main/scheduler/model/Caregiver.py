...
class Caregiver:
    ...
    # getters
    def get(self):
        cm = ConnectionManager() # Create the class cm
        conn = cm.create_connection() #Connects to the db
        cursor = conn.cursor(as_dict=True) # Creates Cursor to interact with the db
